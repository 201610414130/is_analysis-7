...
function addAnotherInput() {
    if (isAllWrite(1)) {
        ...
        var addInputhtml = '<td><span>类型</span><select name="serviceListAfterService[' + index + '].type"><option value="true">增</option><option value="false">退</option></select><span>编号</span><input class="productID" name="orderproductListAfterService[' + index + '].productID" onblur="productIDCheck(this)"></input> <span id="productIDWarn' + index + '"></span> <span>价格</span><input class="sellPrice" name="orderproductListAfterService[' + index + '].sellPrice" onblur="floatNumCheck(this)"></input> <span id="sellPriceWarn' + index + '"></span> <span>件数</span><input class="sellBoxNum" name="orderproductListAfterService[' + index + '].sellBoxNum" onblur="intNumCheck(this)"></input> <span id="sellBoxNumWarn' + index + '"></span> <span>每件几张</span><input class="BoxOwn" name="orderproductListAfterService[' + index + '].BoxOwn" onblur="intNumCheck(this)"></input> <span id="BoxOwnWarn' + index + '"></span> <span>单项</span><input class="sellSingleNum" name="orderproductListAfterService[' + index + '].sellSingleNum" onblur="intNumWithZeroCheck(this)"></input> <span id="sellSingleNumWarn' + index + '"></span> <span>备注</span><input class="remark" name="serviceListAfterService[' + index + '].remark" onblur="remarkCheck(this)"></input> <span id="remarkWarn' + index + '"></span> <button type="button" onClick="removeRow(this)">删除</button></td>';
...
function isAllWrite(isaddInput) {
    ...
    var remarkClass = document.getElementsByClassName("remark");
    var singleNumClass = document.getElementsByClassName("sellSingleNum");
    var state1 = true, state2 = true, state3 = true, state4 = true, state5 = true;
    ...
    if (isaddInput == 0 && state1 && state2 && state3 && state4 && state5) {
...
function remarkCheck(obj) {
    ...
    var indexInput = getInputIndex(obj.name);
    if (stringLength > 100) {
        document.getElementById(obj.className + "Warn" + indexInput).innerHTML = "最多100个字";
        obj.value = "";
    } else {
        document.getElementById(obj.className + "Warn" + indexInput).innerHTML = "";
    }
